...
from .models import Post, Friendship, PostLike, PostComment, Notification, Chat, Message
from .forms import CustomUserCreationForm
from django.db.models import Q, Count, F
from groups.models import GroupPost, GroupSubscription, GroupPostLike, GroupPostComment
...
def index(request):
    ...
    # Если пользователь не авторизован или не подписан на группы - показываем популярные посты
    show_popular = False
    if not request.user.is_authenticated:
        show_popular = True
    else:
        # Проверяем, подписан ли пользователь хотя бы на одну группу
        has_subscriptions = GroupSubscription.objects.filter(
            user=request.user, 
            is_subscribed=True
        ).exists()
    ...
    if show_popular:
        # Показываем популярные посты (по лайкам + комментариям)
        # Посты пользователей
        user_posts = Post.objects.annotate(
            likes_count=Count('likes'),
            comments_count=Count('comments'),
            total_engagement=F('likes_count') + F('comments_count')
        ).select_related('author', 'wall_owner').order_by('-total_engagement', '-created')[:20]
        
        # Посты групп
        group_posts = GroupPost.objects.annotate(
            likes_count=Count('likes'),
            comments_count=Count('comments'),
            total_engagement=F('likes_count') + F('comments_count')
        ).select_related('author', 'group').order_by('-total_engagement', '-created')[:20]
        ...
        # Показываем посты из подписок пользователя
        subscribed_groups = GroupSubscription.objects.filter(
            user=request.user,
            is_subscribed=True
        ).values_list('group_id', flat=True)
        
        # Посты из подписанных групп
        group_posts = GroupPost.objects.filter(
            group_id__in=subscribed_groups
        ).select_related('author', 'group').order_by('-created')[:20]
        
        # Посты пользователей (можно добавить фильтр по друзьям)
        user_posts = Post.objects.select_related('author', 'wall_owner').order_by('-created')[:20]
    ...
    # Добавляем информацию о лайках и комментариях для каждого поста
    for item in all_posts:
        if request.user.is_authenticated:
            if item['type'] == 'group':
                item['is_liked'] = item['post'].is_liked_by(request.user)
                item['likes_count'] = item['post'].get_likes_count()
                item['comments_count'] = item['post'].get_comments_count()
                # Получаем последние комментарии
                item['comments'] = item['post'].comments.select_related('author').all()[:5]
            else:
                item['is_liked'] = item['post'].is_liked_by(request.user)
                item['likes_count'] = item['post'].get_likes_count()
                item['comments_count'] = item['post'].get_comments_count()
    ...
    if request.user.is_authenticated:
        ...
        for chat in user_chats:
            ...
            if other_user:
                total_unread_chats += chat.messages.filter(sender=other_user, read=False).count()
    
    return render(request, 'index.html', {
        'all_posts': all_posts,
        'show_popular': show_popular,
        'unread_notifications': unread_notifications,
        'total_unread': total_unread_chats
    })
...
def profile(request):
    ...
    user_posts = Post.objects.filter(author=request.user).select_related('author', 'wall_owner').order_by('-created')[:10]
...
def user_profile(request, username):
    ...
    try:
        ...
        user_posts = Post.objects.filter(author=profile_user).select_related('author', 'wall_owner').order_by('-created')[:10]
...
def delete_account(request):
    ...
    if request.method == 'POST':
        ...
        if confirm == 'DELETE':
            ...
            logout(request)
            user.delete()  # CASCADE удалит все связанные данные
...
def friends_page(request):
    ...
    if search_query:
        search_results = User.objects.filter(
            username__icontains=search_query
        ).exclude(id=request.user.id).select_related('profile').distinct()[:10]
    ...
    if request.method == 'POST':
        # Обработка действий с друзьями (добавление, принятие, удаление)
        if 'add_friend' in request.POST:
            ...
            friend_id = request.POST.get('accept_friend')
            try:
                friendship = Friendship.objects.get(id=friend_id, to_user=request.user, accepted=False)
                friendship.accepted = True
                friendship.save()
                # Уведомление
                Notification.objects.create(
